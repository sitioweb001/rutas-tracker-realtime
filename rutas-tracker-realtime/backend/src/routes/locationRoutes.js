...
// List ALL last locations (public)
locationRouter.get('/', (req,res)=>{
  const arr = Array.from(store.lastLocations.entries()).map(([transportId, data])=>data);
  res.json(arr);
});

// Last location per transport (public)
...
// Current online/offline state (public)
...
// Tracker stops explicitly -> offline immediately
locationRouter.post('/stop', authRequired, (req,res)=>{
  const user = req.user;
  if(user.role !== 'tracker') return res.status(403).json({ error:'Solo trackers' });
  const transportId = user.transportId;
  if(onlineTimers.has(transportId)) { clearTimeout(onlineTimers.get(transportId)); onlineTimers.delete(transportId); }
  const io = req.app.get('io');
  io.to(`transport:${transportId}`).emit('status', { transportId, online:false, ts: Date.now() });
  res.json({ ok:true });
});
