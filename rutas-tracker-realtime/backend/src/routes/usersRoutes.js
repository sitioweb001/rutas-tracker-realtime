...
// set/update tracker's display name
...
usersRouter.get('/public/labels', (req,res)=>{
  ...
  const trackers = store.users.filter(u=>u.role==='tracker').map(({ email, transportId, displayName })=>({ email, transportId, displayName: displayName || null }));
