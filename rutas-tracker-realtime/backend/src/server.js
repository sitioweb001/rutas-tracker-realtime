...
io.on('connection', (socket)=>{
  socket.on('join', ({transportId})=>{ if(!transportId) return; socket.join(`transport:${transportId}`); socket.emit('joined',{room:`transport:${transportId}`}); });
  socket.on('leave', ({transportId})=>{ if(!transportId) return; socket.leave(`transport:${transportId}`); });
});
