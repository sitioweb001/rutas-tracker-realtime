...
(function(){
  ...
  // Colores
  const colorById = {
    t1:'#22c55e', t2:'#3b82f6', t3:'#f59e0b', t4:'#e11d48', t5:'#06b6d4',
    t6:'#a855f7', t7:'#10b981', t8:'#f97316', t9:'#84cc16', t10:'#0ea5e9',
    t11:'#9333ea', t12:'#ef4444', t13:'#14b8a6', t14:'#8b5cf6', t15:'#f59e0b',
    t16:'#22c55e', t17:'#3b82f6', t18:'#f97316', t19:'#84cc16', t20:'#0ea5e9',
    other:'#64748b'
  };
    ...
    if (toggleBtn) {
      toggleBtn.onclick = () => {
        ...
        toggleBtn.textContent = panel.classList.contains('hidden')
          ? 'Desplegar estado de conductores'
          : 'Ocultar estado de conductores';
      };
    }
  }

  // Alias públicos
  let labelByT = new Map();   // transportId -> alias de transporte
  let driverByT = new Map();  // transportId -> alias de conductor
  let emailByT  = new Map();  // transportId -> email del conductor
  ...
  fetch('/api/users/public/labels')
    ...
    .then(({transports,trackers})=>{
      ...
      trackers.forEach(u=>{
        if(u && u.transportId){
          if(u.displayName) driverByT.set(u.transportId, u.displayName);
          if(u.email)       emailByT.set(u.transportId, u.email);
        }
      });
      // actualizar tooltips e items si ya existían
      markers.forEach((m,tid)=> m.setTooltipContent(titleForTransport(tid)));
      Array.from((panel||document).querySelectorAll('.fleetItem')).forEach(el=>{
        const tid = el.dataset.tid;
        const nameEl = el.querySelector('.fleetName');
        if (nameEl) nameEl.textContent = titleForTransport(tid);
      });
    });
  ...
  function ensureItem(tid){
    ...
    el.dataset.tid = tid;
    el.innerHTML = `
      <span class="fleetDot" style="background:${dotColor}"></span>
  ...
  function setItemStatus(tid, online){
    const el = ensureItem(tid);
    const st = el.querySelector('.fleetStatus');
  ...
  function setItemTime(tid, ts){
    const el = ensureItem(tid);
    const tt = el.querySelector('.fleetTime');
    tt.textContent = ' · ' + (ts? fmtTime(ts) : '—');
  }
  ...
  function upsertMarker(tid, lat, lng){
    ...
    if(!m){
      ...
      m.bindTooltip(titleForTransport(tid), { permanent:true, direction:'top', offset:[0,-10], opacity:.9 });
      markers.set(tid, m);
    }else{
      m.setLatLng([lat,lng]);
      m.setTooltipContent(titleForTransport(tid));
    }
  }

  // Chip última posición (solo 1 transporte)
  const lastFixChip = document.getElementById('lastFix');
  let lastTs=0,lastLat=null,lastLng=null;
  function updateChip(){
    if(isAll){ lastFixChip.style.display='none'; return; }
    if(!lastTs||lastLat==null||lastLng==null){ lastFixChip.style.display='none'; return; }
    lastFixChip.innerHTML = `
      <div><b>Última posición</b></div>
      <div>Lat: ${lastLat.toFixed(6)}<br>Lng: ${lastLng.toFixed(6)}</div>
      <div><small>${new Date(lastTs).toLocaleString()}</small></div>
    `;
  ...
  async function initSingle(){
    ...
    try{
      ...
      if(r.ok){
        ...
        lastTs=d.ts||Date.now(); lastLat=d.lat; lastLng=d.lng;
        lastTsById.set(transportParam,lastTs);
        upsertMarker(transportParam,d.lat,d.lng);
        map.setView([d.lat,d.lng],16);
        updateChip();
        setItemTime(transportParam,lastTs);
      }
    }catch{}
    try{
      const r=await fetch(`/api/location/status/${encodeURIComponent(transportParam)}`);
      const s=r.ok?await r.json():{online:false};
      setItemStatus(transportParam, !!s.online);
    }catch{ setItemStatus(transportParam,false); }
  }
  ...
  async function initAll(){
    ...
    try{
      ...
      if(r.ok){
        ...
        arr.forEach(d=>{
          if(d && d.transportId){
            lastTsById.set(d.transportId, d.ts||Date.now());
            setItemTime(d.transportId, d.ts);
            upsertMarker(d.transportId,d.lat,d.lng);
          }
        });
        const first=arr.find(Boolean);
        if(first) map.setView([first.lat, first.lng], 13);
      }
    }catch{}
    transports.forEach(async t=>{
      try{
        const r=await fetch(`/api/location/status/${encodeURIComponent(t.id)}`);
        const s=r.ok?await r.json():{online:false};
        setItemStatus(t.id, !!s.online);
      }catch{ setItemStatus(t.id,false); }
    });
  }
  ...
  socket.on('location', (p)=>{
    if(!p||!p.transportId) return;
    if(!isAll && p.transportId!==transportParam) return;
    upsertMarker(p.transportId,p.lat,p.lng);
    setItemStatus(p.transportId,true);
    setItemTime(p.transportId,p.ts);
    lastTsById.set(p.transportId,p.ts||Date.now());
    if(!isAll){
      if(p.ts && p.ts<lastTs) return;
      lastTs=p.ts||Date.now(); lastLat=p.lat; lastLng=p.lng;
