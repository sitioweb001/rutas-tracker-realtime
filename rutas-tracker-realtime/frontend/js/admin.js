...
async function adminLogin(){
  ...
  // load transports and trackers
  loadTransports(); loadTrackers();
}
async function loadTransports(){
  const sel = document.getElementById('tselect'); sel.innerHTML = '';
  const r = await fetch('/api/transports/public');
  const list = r.ok ? await r.json() : [];
  list.forEach(t=>{ const opt=document.createElement('option'); opt.value=t.id; opt.textContent=`${t.id} — ${t.name}`; sel.appendChild(opt); });
}
async function loadTrackers(){
  ...
  const list = r.ok ? await r.json() : [];
  list.forEach(u=>{ const opt=document.createElement('option'); opt.value=u.id; opt.textContent=`${u.email} (asig: ${u.transportId||'-'})`; sel.appendChild(opt); });
}
